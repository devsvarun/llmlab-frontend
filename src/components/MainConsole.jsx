...
import { Beaker, Download, Sparkles, BarChart3, Zap, TrendingUp } from "lucide-react";
...
export default function MainConsole() {
    ...
    const generateResponses = async () => {
        ...
        for (let i = 0; i < numVariations[0]; i++) {
            ...
            params.push({
                temperature: parseFloat(Math.min(maxTemp, Math.max(minTemp, tempVar)).toFixed(2)),
                top_p: parseFloat(Math.min(maxTopP, Math.max(minTopP, topPVar)).toFixed(2)),
            });
        }
    ...
    const getBestResponse = () => {
        ...
        const best = responses.reduce((best, current) => {
            return current?.metrics?.avg_score > best?.metrics?.avg_score ? current : best;
        });
        return best;
    };
                    ...
                    <div className="flex items-center justify-between h-16">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500">Parameter Experimentation Console</p>
                            </div>
                        </div>
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <Card className="border-2 hover:border-indigo-200 transition-all duration-300">
                            ...
                            <CardContent className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <Badge variant="secondary">{temperature[0].toFixed(2)}</Badge>
                                        </div>
                                        <Slider
                                            value={temperature}
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <Slider value={topP} onValueChange={setTopP} min={0} max={1} step={0.05} className="cursor-pointer" />
                                        <p className="text-xs text-slate-500">Nucleus sampling threshold</p>
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <Slider
                                            ...
                                            className="cursor-pointer"
                                        />
                                        <p className="text-xs text-slate-500">Number of responses to generate</p>
                                    </div>
                                </div>
                                ...
                                <input type="file" accept="application/json" id="importFile" hidden disabled={isGenerating} onChange={handleImport} />
                            ...
                            {responses.length === 0 ? (
                                <Card className="border-2 border-dashed border-slate-200">
                                    <CardContent className="flex flex-col items-center justify-center py-20 text-center">
                                        ...
                                        <h3 className="text-xl font-semibold text-slate-700 mb-2">No experiments yet</h3>
                                        <p className="text-slate-500 max-w-md">
                                            Configure your parameters and generate responses to see analysis and comparisons
                                        </p>
                                    </CardContent>
                                </Card>
                            ) : (
                                                ...
                                                <CardContent>
                                                    ...
                                                    <p className="text-sm text-slate-700 leading-relaxed mb-4">
                                                        <div className="prose prose-neutral max-w-none">
                                                            <ReactMarkdown>{getBestResponse().response}</ReactMarkdown>
                                                        </div>
                                                    </p>
                                                    <div className="grid sm:grid-cols-4 grid-cols-2 gap-3">
                                                        {Object.entries(getBestResponse().metrics).map(([key, value]) => (
                                                            <div key={key} className="text-center p-2 bg-white rounded-lg">
                                                                <div className={`text-lg font-bold ${getMetricColor(value)}`}>
                                                                    {(value * 100).toFixed(0)}%
                                                                </div>
                                                                <div className="text-wrap text-xs text-slate-500 capitalize">{key}</div>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </CardContent>
                                            ...
                                            >
                                                <CardHeader>
                                                    <div className="flex items-center justify-between">
                                                        <CardTitle className="text-base">Response #{responses.indexOf(response) + 1}</CardTitle>
                                                ...
                                                <CardContent className="space-y-4">
                                                    ...
                                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                                        {Object.entries(response.metrics).map(([key, value]) => (
                                                            ...
                                                            >
                                                                <div className={`text-xl font-bold ${getMetricColor(value)}`}>
                                                                    {(value * 100).toFixed(0)}%
                                                                </div>
                                    ...
                                    <TabsContent value="comparison">
                                        <Card>
                                            <CardHeader>
                                                ...
                                                <CardDescription>Side-by-side analysis of all responses</CardDescription>
                                            </CardHeader>
                                            <CardContent>
                                                            ...
                                                            >
                                                                <thead>
                                                                    <tr className="border-b border-slate-200">
                                                                        ...
                                                                        <th className="text-left p-3 font-semibold text-sm">Top-P</th>
                                                                        <th className="text-left p-3 font-semibold text-sm">Coherence</th>
                                                                        <th className="text-left p-3 font-semibold text-sm">Relevance</th>
                                                                        <th className="text-left p-3 font-semibold text-sm">Completeness</th>
                                                                        <th className="text-left p-3 font-semibold text-sm">Diversity</th>
                                                                        <th className="text-left p-3 font-semibold text-sm">Avg Score</th>
                                                                    </tr>
                                                                </thead>
                                                                <tbody>
                                                                    {responses.map((response, idx) => {
                                                                        const avgScore = response.metrics.avg_score.toFixed(2);
                                                                        return (
                                                                            <tr
                                                                                key={response.id}
                                                                                className="border-b border-slate-100 hover:bg-slate-50 transition-colors"
                                                                            >
                                                                                <td className="p-3 font-medium align-top w-10">{idx + 1}</td>

                                                                                <td className="p-3 align-top w-24 break-words">
                                                                                    {response.temperature}
                                                                                </td>

                                                                                <td className="p-3 align-top w-24 break-words">{response.topP}</td>
                                                                                ...
                                                                                >
                                                                                    {(response.metrics.coherence * 100).toFixed(0)}%
                                                                                </td>
                                                                                ...
                                                                                >
                                                                                    {(response.metrics.relevance * 100).toFixed(0)}%
                                                                                </td>
                                                                                ...
                                                                                >
                                                                                    {(response.metrics.completeness * 100).toFixed(0)}%
                                                                                </td>
                                                                                ...
                                                                                >
                                                                                    {(response.metrics.diversity * 100).toFixed(0)}%
                                                                                </td>

                                                                                <td className="p-3 align-top w-24">
                                                                                    <Badge
                                                                                        variant={
                                                                                            parseFloat(avgScore) >= 0.75 ? "default" : "secondary"
                                                                                        }
                                                                                    >
                                                                                        {(parseFloat(avgScore) * 100).toFixed(0)}%
                                                                                    </Badge>
                                                                                </td>
                                                                            </tr>
                                                                        );
                                                    ...
                                                    {responses.map((response, idx) => (
                                                        <div key={response.id} className="border rounded-lg p-3 bg-white">
                                                            <div className="flex justify-between items-start mb-2">
                                                                <div className="font-medium">Response #{idx + 1}</div>
                                                                <div className="text-xs text-slate-500">
                                                                    Temp: {response.temperature} • Top-P: {response.topP}
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div className="grid grid-cols-2 gap-2">
                                                                <div className="p-2 bg-slate-50 rounded">
                                                                    <div
                                                                        className={`text-lg font-bold ${getMetricColor(response.metrics.coherence)}`}
                                                                    >
                                                                        {(response.metrics.coherence * 100).toFixed(0)}%
                                                                    </div>
                                                                ...
                                                                <div className="p-2 bg-slate-50 rounded">
                                                                    <div
                                                                        className={`text-lg font-bold ${getMetricColor(response.metrics.relevance)}`}
                                                                    >
                                                                        {(response.metrics.relevance * 100).toFixed(0)}%
                                                                    </div>
                                                                ...
                                                                <div className="p-2 bg-slate-50 rounded">
                                                                    <div
                                                                        className={`text-lg font-bold ${getMetricColor(response.metrics.diversity)}`}
                                                                    >
                                                                        {(response.metrics.diversity * 100).toFixed(0)}%
                                                                    </div>
